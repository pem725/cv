...
```{python}
#| output: asis
for h in sorted(data.get('honors', []), key=lambda x: x.get('year', 0), reverse=True):
    org = f" — {h['org']}" if h.get('org') else ""
    print(f"**{h['year']}.** {h['title']}{org}\n")
```
...
for article in articles:
    ...
    if 'journal' in article:
        ...
        if 'volume' in article:
            ...
            if 'issue' in article:
                cite += f"({article['issue']})"
        if 'pages' in article:
            cite += f", {article['pages']}"
        cite += "."
    if article.get('status'):
        cite = cite.rstrip('.') + f" ({article['status']})."
    if article.get('doi'):
        cite += f" [doi:{article['doi']}](https://doi.org/{article['doi']})"
...
```{python}
#| output: asis
proceedings = sorted(data['publications'].get('conference_proceedings', []), key=lambda x: x['year'], reverse=True)
for p in proceedings:
    print(f"- {p['authors']} ({p['year']}). {p['title']}. *{p['venue']}*.\n")
```
...
```{python}
#| output: asis
for m in data['publications'].get('under_review', []):
    auth = m['authors'].rstrip()
    yr = f" ({m['year']})" if m.get('year') else ""
    venue = m.get('venue', '')
    at = f" *{venue}*." if venue and venue.lower() != 'under review' else " *Under review*."
    note = f" [{m['note']}]" if m.get('note') else ""
    print(f"- {auth}{yr}. {m['title']}.{at}{note}\n")
```

<!-- historical_submitted / historical_in_preparation in publications.yaml are archival
     (some ~20 years old, never appeared) and are intentionally NOT rendered. -->
...
```{python}
#| output: asis
for p in sorted(data['publications'].get('preprints', []), key=lambda x: x.get('year', 0), reverse=True):
    auth = p['authors'].rstrip('.')
    venue = f" *{p['venue']}*." if p.get('venue') else ""
    link = f" [doi:{p['doi']}](https://doi.org/{p['doi']})" if p.get('doi') else ""
    print(f"- {auth} ({p['year']}). {p['title']}.{venue}{link}\n")
```
...
```{python}
#| output: asis
for s in data['publications'].get('software', []):
    url = f" <{s['url']}>" if s.get('url') else ""
    print(f"- **{s['name']}** — {s['desc']}.{url}\n")
```
