...
<a href="mailto:pmcknigh@gmu.edu" class="hero-link">📧 Email</a>
<a href="https://psychology.gmu.edu/people/pmcknigh" class="hero-link">🏛️ GMU</a>
<a href="https://scholar.google.com/citations?user=sH44LC4AAAAJ" class="hero-link">📚 Scholar</a>
<a href="https://orcid.org/0000-0002-9067-9066" class="hero-link">🔗 ORCID</a>
</div>
</div>
...
<div class="pos-dates">{pos['start']} – {end_str}</div>
</div>
""")
print('</div>')
```
...
for book in data['publications']['books']:
    ...
    if 'citations' in book:
        print(f'<span class="citation-badge">📖 {book["citations"]:,} citations</span>')
    print('</div>')
```
...
for article in articles[:15]:  # Show top 15
    ...
    if 'citations' in article:
        print(f'<span class="citation-badge">📊 {article["citations"]:,} citations</span>')
    print('</div>')
```

<p class="more-link"><a href="https://scholar.google.com/citations?user=sH44LC4AAAAJ">View all publications on Google Scholar →</a></p>
...
<div class="grant-dates">{grant['start']} – {grant['end']}</div>
</div>
""")
print('</div>')
```
...
if 'personal' in data:
    ...
    if 'athletics' in personal and 'mountaineering' in personal['athletics']:
        ...
        print('<div class="personal-section">')
        print('<h3>🏔️ Mountaineering</h3>')
        print('<ul>')
        for item in mountains:
            if 'summit' in item:
                print(f'<li><strong>{item["summit"]}</strong> ({item.get("year", "")}) — {item.get("type", "")}</li>')
            elif 'achievement' in item:
                print(f'<li>{item["achievement"]}</li>')
        print('</ul>')
        print('</div>')
    ...
    if 'athletics' in personal and 'triathlon' in personal['athletics']:
        ...
        print('<div class="personal-section">')
        print('<h3>🏃 Triathlon</h3>')
        print('<ul>')
        for item in tri:
            if 'event' in item:
                print(f'<li><strong>{item["event"]}</strong> — {item.get("time", "")} (Top {item.get("placement", "")})</li>')
        print('</ul>')
        print('</div>')
    ...
    if data.get('erdos_number'):
        print(f'<p class="erdos">Erdős Number: <strong>{data["erdos_number"]}</strong></p>')
```
